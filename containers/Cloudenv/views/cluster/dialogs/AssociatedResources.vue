<template>
  <base-dialog @cancel="cancelDialog">
    ...
    <div slot="body">
      <dialog-selected-tips :name="$t('dictionary.hostcluster')" :count="params.data.length" :action="$t('cloudenv.text_454')" />
...
export default {
  name: 'AssociatedHostResourcesDialog',
  ...
  methods: {
    ...
    async fetchBindedResources () {
      try {
        const { data: { data = [] } } = await new this.$Manager('clusters').get({
